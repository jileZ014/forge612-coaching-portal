    ...
    <div className="min-h-screen bg-[#0A0A0A]">
      <header className="bg-[#141418] border-b border-white/[0.09]">
        <div className="max-w-3xl mx-auto px-6 py-4 flex items-center gap-3">
          ...
          <h1 className="text-2xl font-semibold text-white">New Family</h1>
        </div>
      </header>
      ...
      <main className="max-w-3xl mx-auto px-6 py-6">
        <form onSubmit={submit} className="space-y-6 animate-fade-up">
          <section className="bg-[#141418] rounded-md border border-white/[0.09] p-5">
            <h2 className="font-semibold text-white mb-4">Primary parent / guardian</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Field label="Name *">
                <input
                  ...
                  className="w-full px-3 py-2 rounded-md border border-white/[0.09] focus:border-white/30 focus:outline-none text-white placeholder:text-white/35"
                />
              </Field>
              <Field label="Email *">
                <input
          ...
          <section className="bg-[#141418] rounded-md border border-white/[0.09] p-5">
            <h2 className="font-semibold text-white mb-4">
              Secondary parent / guardian <span className="text-white/35 text-sm">(optional)</span>
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Field label="Name">
                <input
          ...
          <section className="bg-[#141418] rounded-md border border-white/[0.09] p-5">
            ...
            {players.map((p, idx) => (
              <div key={idx} className="grid grid-cols-1 md:grid-cols-5 gap-3 mb-3 pb-3 border-b border-white/[0.06] last:border-0 last:mb-0 last:pb-0">
                <input
                  required={idx === 0}
                  value={p.name}
                  onChange={(e) => updatePlayer(idx, { name: e.target.value })}
                  placeholder="Player name *"
                  className="md:col-span-2 px-3 py-2 rounded-md border border-white/[0.09] focus:border-white/30 focus:outline-none text-white placeholder:text-white/35"
                />
                <input
                  value={p.position}
                  onChange={(e) => updatePlayer(idx, { position: e.target.value })}
                  placeholder="Position"
                  className="px-3 py-2 rounded-md border border-white/[0.09] focus:border-white/30 focus:outline-none text-white placeholder:text-white/35"
                />
                <input
                  type="number"
                  value={p.birthYear}
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    placeholder="Grad yr"
                    className="flex-1 px-3 py-2 rounded-md border border-white/[0.09] focus:border-white/30 focus:outline-none text-white placeholder:text-white/35"
                  />
                  {players.length > 1 && (
                    <button
                      type="button"
                ...
                <input
                  ...
                  placeholder="School"
                  className="md:col-span-5 px-3 py-2 rounded-md border border-white/[0.09] focus:border-white/30 focus:outline-none text-white placeholder:text-white/35"
                />
              </div>
            ))}
          </section>
          ...
          <section className="bg-[#141418] rounded-md border border-white/[0.09] p-5">
            <h2 className="font-semibold text-white mb-4">Onboarding details</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Field label="Starting lifecycle stage">
                <select
                  value={stage}
                  onChange={(e) => setStage(e.target.value as LifecycleStage)}
                  className="w-full px-3 py-2 rounded-md border border-white/[0.09] focus:border-white/30 focus:outline-none text-white placeholder:text-white/35 bg-[#141418]"
                >
          ...
          {error && (
            <div className="bg-red-500/10 border border-red-400/30 text-red-200 rounded-md p-4 text-sm">
              {error}
            </div>
          )}
          ...
          <div className="flex justify-end gap-3">
            <Link
              ...
              className="px-4 py-2 rounded-md border border-white/15 text-sm font-medium text-white/70 hover:bg-[#141418]/10"
            >
              Cancel
            </Link>
            <button
